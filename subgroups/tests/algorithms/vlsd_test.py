...
def test_VLSD_init_method_1():
    dictionary = dict()
    vlsd = VLSD(WRAcc(), -1, WRAccUpperBound1(), -1, additional_parameters_for_the_quality_measure=dictionary, additional_parameters_for_the_upper_bound=dictionary)
...
def test_VLSD_init_method_2():
    vlsd = VLSD(WRAcc(), -1, WRAccUpperBound1(), -0.85)
    try:
        vlsd = VLSD(Qg(), -1, WRAccUpperBound1(), -0.85, additional_parameters_for_the_quality_measure={"Qg" : 0.2})
        assert (False)
    except ValueError:
        assert (True)
...
def test_VLSD_fit_method_2():
    ...
    target = ("class", "y")
    vlsd = VLSD(WRAcc(), -1, WRAccUpperBound1(), -1) # IMPORTANT: WRAcc quality measure is defined between -1 and 1.
...
def test_VLSD_fit_method_3():
    df = DataFrame({"a1" : ["a","b","c","c"], "a2" : ["q","q","s","q"], "a3" : ["f","g","h","k"], "class" : ["n","y","n","y"]})
    target = ("class", "y")
    vlsd = VLSD(WRAcc(), 0, WRAccUpperBound1(), 0) # IMPORTANT: WRAcc quality measure is defined between -1 and 1.
    subgroups = vlsd.fit(df, target)
    final_tuples = []
    for elem in subgroups:
        final_tuples.append( (elem[0],round(elem[1], 3)) )
...
def test_VLSD_fit_method_4():
    df = DataFrame({"a1" : ["a","b","c","c"], "a2" : ["q","q","s","q"], "a3" : ["f","g","h","k"], "class" : ["n","y","n","y"]})
    target = ("class", "y")
    vlsd = VLSD(WRAcc(), 0.1, WRAccUpperBound1(), 0.1) # IMPORTANT: WRAcc quality measure is defined between -1 and 1.
    subgroups = vlsd.fit(df, target)
    final_tuples = []
    for elem in subgroups:
        final_tuples.append( (elem[0],round(elem[1], 3)) )
...
def test_VLSD_fit_method_5():
    df = DataFrame({"a1" : ["a","b","c","c"], "a2" : ["q","q","s","q"], "a3" : ["f","g","h","k"], "class" : ["n","y","n","y"]})
    target = ("class", "y")
    vlsd = VLSD(WRAcc(), 0.1, WRAccUpperBound1(), 0.1, additional_parameters_for_the_quality_measure={"TP" : 100000, "g" : 0.5}, additional_parameters_for_the_upper_bound={"FP" : 100000, "g" : 0.4}) # IMPORTANT: the subgroup parameters TP and FP must be deleted in the __init__ method.
    assert (len(vlsd.additional_parameters_for_the_quality_measure) == 1)
    subgroups = vlsd.fit(df, target)
    final_tuples = []
    for elem in subgroups:
