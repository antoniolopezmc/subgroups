...
class VLSD(Algorithm):
    ...
    :type additional_parameters_for_the_upper_bound: dict
    ...
    # IMPORTANT: to avoid to compute the subgroup parameters TP and FP twice, we pass them by parameter in this method.
    def _generate_initial_list_of_vertical_lists(self, pandas_dataframe, TP, FP, target):
        ...
        :type TP: int
        :param TP: the True Positives of the dataset. IMPORTANT: although it can be computed from 'pandas_dataframe', we pass it by parameter to avoid compute it twice (in the 'fit' method and in this method).
        :type FP: int
        :param FP: the False Positives of the dataset. IMPORTANT: although it can be computed from 'pandas_dataframe', we pass it by parameter to avoid compute it twice (in the 'fit' method and in this method).
        ...
        :return: a list in which each element is a Vertical List which represents a single selector (i.e., it only has one selector in its list of selectors). The list is pruned according to the threshold and sorted descending by the quality value of each Vertical List.
        """
        ...
        for column in pandas_dataframe.columns.drop(target[0]):
            ...
            values_and_target_grouped = pandas_dataframe.groupby([column, target_attribute_as_a_mask])
            ...
            for value_target_tuple in values_and_target_grouped:
                ...
                if value not in processed_values:
                    ...
                    # Pruning: add the Vertical List only if the quality measure value is greater or equal than the threshold.
                    if quality_measure_value >= self._minimum_threshold:
                        # Create the Vertical List.
                        vl = VerticalList([Selector(column, Operator.EQUAL, value)], registers_tp, registers_fp, quality_measure_value)
    ...
    def _search(self, vl, P, M, TP, FP, target): #### CORREGIR: orden de selectores al consultar M.
        ...
        for index_x in range(len(P)):
            ...
            if (Px != P[-1]) and ((vl_quality_value + M[ea][ea]) >= self._minimum_threshold):
                ...
                for index_y in range(index_x+1, len(P)): # IMPORTANT: x < y
                    ...
                    if (M[ea][eb] >= self._minimum_threshold):
                        ...
                        dict_of_parameters.update(self._additional_parameters_for_the_upper_bound)
                        # Make the union.
                        new_vl_union = Px.union(Py, self._upper_bound, dict_of_parameters)
                ...
                if V:
                    ...
                    F = F + self._search(Px, V, M, TP, FP, target)
        # Return the result.
        return F
    ...
    def fit(self, pandas_dataframe, target):
        ...
        for index_x in range(len(P)):
            ...
            for index_y in range(index_x, len(P)): # IMPORTANT: x <= y
                ...
                vl_xy_dict_of_parameters = {QualityMeasure.SUBGROUP_PARAMETER_tp : len(vl_xy_sequence_of_instances_tp), QualityMeasure.SUBGROUP_PARAMETER_fp : len(vl_xy_sequence_of_instances_fp), QualityMeasure.SUBGROUP_PARAMETER_TP : TP, QualityMeasure.SUBGROUP_PARAMETER_FP : FP}
                vl_xy_dict_of_parameters.update(self._additional_parameters_for_the_upper_bound)
                # Add to the dictionary.
                M[ea] = dict()
                M[ea][eb] = self._upper_bound.compute(vl_xy_dict_of_parameters) # IMPORTANT: M[ea][eb] is equal to M[eb][ea], but only one entry is added (to save memory). This will have to be kept in mind later.
        # Call to the search method.
        F = self._search(None, P, M, TP, FP, target)
        # Iterate through the result (list F).
        for vl in F:
            # Create the subgroup and compute its quality measire according to 'quality_measure' attribute.
            s = Subgroup(vl.list_of_selectors, target)
