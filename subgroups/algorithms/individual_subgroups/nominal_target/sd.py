...
class SD(Algorithm):
   ...
   # __slots__ =( "g_parameter,_minimum_quality_measure_value","_quality_measure","_additional_parameters_for_the_quality_measure", "beam_width","_unselected_subgroups", "_selected_subgroups", "_file_path", "_file")
    ...
    def _handle_individual_result(self, individual_result: tuple[Pattern, tuple[str, str], int, int, int, int], quality_measure : QualityMeasure) -> float:
       ...
       # print(individual_result[0])
        ...
        if quality_measure_value >= self._get_minimum_quality_measure_value():
            ...
            if self._file_path is not None:
                ...
                self._file.write("Quality Measure " + str(quality_measure.get_name()) + " = " + str(quality_measure_value) + " ; ")
        ...
        while improvements_in_beam:
            ...
            for index in range(self._get_beam_width()):
                ...
                for l in set_l:
                    ...
                    current_subgroup_in_beam_qg = self._handle_individual_result((Pattern([current_subgroup_in_beam]),tuple_target_attribute_value,current_subgroup_in_beam_BasicMetrics),Qg())
                    beam[index][1] = current_subgroup_in_beam_qg # Update the Qg quality measure (second element in the tuple).
                    current_subgroup_in_beam_support = self._handle_individual_result((Pattern([current_subgroup_in_beam]),tuple_target_attribute_value,current_subgroup_in_beam_BasicMetrics),Support())
